...
# --- 課題1：データの保存・読み込みの仕組み（CSVファイルで永続化） ---
...
if not df_saved.empty:
    ...
    for i, event in enumerate(events):
        with tabs[i]:
            ...
            # --- 課題2：自己ベスト・シーズンベストの自動表示 ---
            # トラック種目かフィールド種目かを判定してベストを決める簡易ロジック
            # (100mなどのタイムは小さい方が良い、走幅跳など'跳'や'投'がつくものは大きい方が良い)
            is_field = "跳" in event or "投" in event
            
            # 文字列を比較用の数値に変換する関数
            def to_num(val):
                try:
                    if ":" in str(val): # 1500mなどの 4:15.00 対策
                        parts = str(val).split(":")
                        return float(parts[0]) * 60 + float(parts[1])
                    return float(str(val).replace("m", "")) # フィールドの 5m30 などの対策
                except:
            ...
            if is_field:
                ...
                # 今年のデータ（日付に2026が含まれるもの、など）からシーズンベストを抽出
                # ここでは仮に全データから最新年のものをシーズンベストとします
            ...
            # トラック種目の場合はグラフの縦軸（Y軸）を上下反転させて、速い記録が上に来るようにする
            if not is_field:
